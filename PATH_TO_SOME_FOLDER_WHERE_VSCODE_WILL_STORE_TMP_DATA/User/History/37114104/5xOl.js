...
app.get('/newsfeed', async (req, res) => {
    ...
    try {
        ...
        // Fetch all users except the logged-in user, including friendRequests
        const users = await collection.find({ email: { $ne: req.session.email } })
            .select('userName profilePic friendRequests');

        // Extract the usernames to whom the current user has sent friend requests
        const friendRequestsSent = user.friendRequests || []; // Ensure it is an array

        res.render('newsfeed', {
            userName: user.userName,
            profilePic: user.profilePic || '/posts/user.png',
            users: users,
            friendRequestsSent: friendRequestsSent // Send this to the frontend
        });
