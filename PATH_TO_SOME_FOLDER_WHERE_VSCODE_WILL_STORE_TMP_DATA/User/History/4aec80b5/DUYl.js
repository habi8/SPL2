...
app.get('/newsfeed', async (req, res) => {
    ...
    try {
        ...
        const friendRequestsSent = user.friendRequests || []; // Ensure this is always an array

        res.render('newsfeed', {
            userName: user.userName,
            profilePic: user.profilePic || '/posts/user.png',
            users: users // ✅ Pass `requests` to the EJS template
        });
...
app.get('/api/friend-requests', async (req, res) => {
    ...
    try {
        ...
        const friendRequestsSent = user.friendRequests || []; // Ensure it’s always an array
        res.json({ requests: friendRequestsSent });
...
app.post('/addFriend', async (req, res) => {
    ...
    try {
        ...
        const result = await collection.updateOne(
            { userName: toUserName },
            { $push: { friendRequests: fromUserName } } // Only adds if not already in the array
        );
