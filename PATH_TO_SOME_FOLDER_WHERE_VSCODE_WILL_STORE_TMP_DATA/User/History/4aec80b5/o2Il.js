...
app.post("/uploadPost", upload.single("photo"), async (req, res) => {
    try {
        ...
        // 1️⃣ First, save post to MongoDB (without image)
        ...
        const savedPost = await newPost.save(); // Save post and get _id

        // 2️⃣ Upload the image to Cloudinary with MongoDB `_id` as filename
        if (req.file) {
            const result = await cloudinary.uploader.upload(req.file.path, {
                folder: "posts", // Upload inside "posts" folder
                public_id: savedPost._id.toString(), // Use MongoDB _id as filename
                overwrite: true, // Replace if re-uploaded
            });

            // 3️⃣ Update the post with the Cloudinary image URL
            savedPost.photo = result.secure_url;
            await savedPost.save();
        }

        res.json({ success: true, message: "Post uploaded successfully!", post: savedPost });
    } catch (error) {
        console.error(error);
