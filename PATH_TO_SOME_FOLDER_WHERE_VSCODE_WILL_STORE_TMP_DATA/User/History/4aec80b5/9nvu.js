...
app.post("/uploadPost", upload.single("photo"), async (req, res) => {
    try {
        ...
        // 1️⃣ First, create a new post object (with placeholder for the photo)
        ...
        // Save the post object in MongoDB (post without photo)
        const savedPost = await newPost.save();

        // 2️⃣ If there is a file, upload it to Cloudinary
        if (req.file) {
            const result = await cloudinary.uploader.upload(req.file.path, {
                folder: "posts", // Upload inside the "posts" folder
                public_id: savedPost.postID.toString(), // Use the MongoDB post `_id` as Cloudinary filename
                overwrite: true, // Overwrite the image if it already exists
            });

            // 3️⃣ After uploading, update the post with the Cloudinary image URL
        ...
        // Send a success response with the saved post
        res.json({
            success: true,
            message: "Post uploaded successfully!",
            post: savedPost, // Send the post back in the response
        });
    } catch (error) {
        console.error(error);
...
app.get('/getAllPosts', async (req, res) => {
    try {
        const posts = await posts.find().sort({ createdAt: -1 }); // Get all posts, sorted by created time (most recent first)
