...
app.post('/newsfeed', async (req, res) => {
    ...
    try {
        ...
        if (isPasswordMatch) {
            ...
            const users = await collection.find({}).select('userName profilePic'); // Only select the fields you need
...
app.get('/newsfeed', async (req, res) => {
    ...
    const users = await collection.find({}, { projection: { userName: 1, profilePic: 1 } }).toArray();
