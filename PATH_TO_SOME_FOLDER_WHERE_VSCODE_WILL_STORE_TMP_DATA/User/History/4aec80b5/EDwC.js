...
app.get('/newsfeed', async (req, res) => {
    ...
    try {
        ...
        const users = await collection.find({ email: { $ne: req.session.email } }).select('userName profilePic friendRequests');
        const userList = await collection.find({ email: { $ne: req.session.email } })
        // Extract usernames to whom the current user has sent requests
        const friendRequestsSent = userList.filter(u => u.friendRequests?.includes(user.userName)).map(u => u.userName);
...
app.post('/addFriend', async (req, res) => {
    ...
    try {
        ...
        const result = await collection.updateOne(
            { userName: toUserName },
            { $addToSet: { friendRequests: fromUserName } } // Only adds if not already in the array
        );
